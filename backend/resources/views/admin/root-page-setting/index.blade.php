@section('title', 'Root Page Setting')
@extends('layouts.app')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="h4 mb-0 fw-bold"><i class="fas fa-file-alt me-2 text-primary"></i>Root Page Setting</h2>
            <small class="text-muted">Region → State → City — banner, SEO meta, FAQs &amp; best time to visit</small>
        </div>
    </div>

    {{-- Tabs --}}
    <ul class="nav nav-tabs mb-4" id="rootPageTabs" role="tablist">
    ...
    <div class="tab-content" id="rootPageTabContent">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                @empty
                                <tr><td colspan="5" class="text-center text-muted py-4">No regions found.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="tab-pane fade {{ request('tab') === 'states' ? 'show active' : '' }}"
            ...
            <div class="card">
                ...
                <div class="card-body p-0" id="stateListWrapper">
                    @include('admin.root-page-setting.state-list', ['states' => $states])
                </div>
            </div>
        </div>
        ...
        <div class="tab-pane fade {{ request('tab') === 'cities' ? 'show active' : '' }}"
            ...
            <div class="card">
                ...
                <div class="card-body p-0" id="cityListWrapper">
                    @include('admin.root-page-setting.city-table', ['locations' => $locations])
                </div>
            </div>
        </div>
...
$('#stateSearch').on('input', function() {
    ...
    stateTimer = setTimeout(function() {
        ...
        $.get("{{ route('admin.root-page-setting.states.search') }}", { search: q })
            .done(function(res) { $('#stateListWrapper').html(res.html); })
...
$(document).on('click', '.btn-city-faq', function() {
    ...
    $.get("{{ route('admin.root-page-setting.index') }}", { faqs: true, id: id })
        .done(function(res) {
...
$(document).on('click', '.btn-city-besttime', function() {
    ...
    $.get("{{ route('admin.root-page-setting.index') }}", { best_times: true, id: id })
        .done(function(res) {
...
$('#citySearch').on('input', function() {
    ...
    cityTimer = setTimeout(function() {
        ...
        $.get("{{ route('admin.root-page-setting.cities.search') }}", { city: q })
            .done(function(res) { $('#cityListWrapper').html(res.html); })
...
document.querySelectorAll('#rootPageTabs button').forEach(function(btn) {
